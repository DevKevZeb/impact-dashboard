  ...
  return (
    <div key={id} className="flex flex-col overflow-hidden rounded-md bg-white shadow-md transition hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-5 px-10">
        <Link to={`/development/projects/${id}`} className="mb-2 line-clamp-2 text-xl font-bold text-secondary hover:text-primary"> {name}</Link>
        <p className="mb-6 line-clamp-5 text-gray-600">
          {description}
        </p>
        <div className="mt-auto">
          <button className="btn-secondary w-full" onClick={() => navigate(`/development/projects/${id}`)} >
            VIEW PROJECT
          </button>
        </div>
      </div>
